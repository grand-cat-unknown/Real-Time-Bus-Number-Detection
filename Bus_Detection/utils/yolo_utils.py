...
def get_cropped(image, out_scores, out_boxes, out_classes, class_names, colors):
    h, w, _ = image.shape
    cropped_images = []
...
def draw_boxes2(image, out_scores, out_boxes, out_classes):
  h, w, _ = image.shape
  cropped_images = []
  for i, c in (list(enumerate(out_classes))):
      predicted_class = out_classes[c]
      # predicted_class = c
      box = out_boxes[i]
      score = out_scores[i]
      ...
      top, left, bottom, right = box
      top = h*max(0, np.floor(top + 0.5).astype('int32'))
      left = w*max(0, np.floor(left + 0.5).astype('int32'))
      bottom = h*min(h, np.floor(bottom + 0.5).astype('int32'))
      right = w*min(w, np.floor(right + 0.5).astype('int32'))
      # print(label, (left, top), (right, bottom))
